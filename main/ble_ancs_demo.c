...
static void periodic_timer_callback(void* arg)
{
    ...
    if (data_buffer.len > 0) {
        esp_receive_apple_data_source(data_buffer.buffer, data_buffer.len, print_message);
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_NOTIFY_EVT:
        ...
        if (param->notify.handle == gl_profile_tab[PROFILE_A_APP_ID].notification_source_handle) {
            ...
            if ((param->notify.value[1] & EventFlagPreExisting) != 0)
            {
                ESP_LOGI(BLE_ANCS_TAG, "Skip this notification");
                return;
            }
...
void app_main(void)
{
  ...
  char count_str[11] = {0};
//   while(1) {
//     snprintf(count_str, 11, "%lu", count++);
...
static void print_message(const char * text){
    ESP_LOGI(BLE_ANCS_TAG, "print_message");
    ESP_LOGI(BLE_ANCS_TAG, "%s", text);
    // Use the semaphore to gain acccess to LVGL
    xSemaphoreTake(xGuiSemaphore, portMAX_DELAY);
